    ...
    void FuzzyPID::defuzz_method()
    {
        for (int k = 0; k < 3; k++) // 第一层循环是为了遍历Kp,Ki,Kd三个模糊集
        {
            ...
            for (int i = 0; i < M; i++) // error
            {
                ...
                {
                    float rule_value = (k == 0) ? kp_rule_matrix[index[i]][index[j + M]] : (k == 1) ? ki_rule_matrix[index[i]][index[j + M]]
                                                                                                    : kd_rule_matrix[index[i]][index[j + M]];
                    float delta = membership[i] * membership[j + M] * rule_value * learn_rate;
                    if (k == 0)
                        delta_kp += delta;
                    if (k == 1)
                        delta_ki += delta;
        ...
        if (fabs(delta_kp) > delta_kp_max)
        {
            delta_kp = delta_kp / fabs(delta_kp_max) * delta_kp_max;
        }
        if (fabs(delta_ki) > delta_ki_max)
        {
            delta_ki = delta_ki / fabs(delta_ki_max) * delta_ki_max;
        }
        if (fabs(delta_kd) > delta_kd_max)
        {
            delta_kd = delta_kd / fabs(delta_kd_max) * delta_kd_max;
        }
    }
} // namespace fuzzy_pid
